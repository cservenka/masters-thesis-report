    ...
    \resizebox{.8\linewidth}{!}{
        \begin{minipage}{\linewidth}
            \begin{alignat*}{6}
                ...
                &\textbf{(15)}\quad&& &&\inst{xor}\quad &&r_m\quad &&r_{sp}\qquad &&\text{; Store address of main object in     $r_m$}\\
...
This \textsc{Pisa} code block initializes the free lists pointer, the heap pointer, the stack pointer, allocates the main object on the stack, calls the main method, deallocates the main object and finally clears the free lists, heap and stack pointers.
...
Figure~\ref{fig:pisa-buddy-outer-if} shows the translation of the outer \textbf{if-then} statement. As briefly mentioned, we can utilize \textsc{Pisa}'s right bit shift instruction, \inst{RL}, in place of the \textbf{double} helper procedure from the \textsc{Janus} implementation. By using a simple bit shift, we are maintain reversibility elegantly when doubling or halving numbers in the power-of-two. This statement also contains one of the careful storage operations of the free list values, in instruction \textbf{(16)}. Before we recursively branch to the entry point, we must place the previously extracted address of the head of the free list back into the free list. This is also the reason for instruction \textbf{(3)} in figure~\ref{fig:buddy-allocation-entry}. Furthermore, we must all temporary evaluated expression values to the stack, so they can be popped when we return.   
...
\begin{figure}[ht]
    ...
    \end{subfigure}%
    \caption{Non-oppsite deallocation results in a different free list after termination}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[ht]{.32\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \filldraw[fill = darkgrey, draw = black] (0, 1.5) rectangle (3, 2) node[midway] {$n$};
...
As \rooplpp arrays also are heap allocated, the buddy allocation implementation is also used for allocating arrays. The only difference between object and array allocation is that no virtual table is stored in the allocated space but the offsets for the reference counter are equal. Due to this fact, \textbf{copy} and \textbf{uncopy} \textsc{Pisa} blocks generated during compile time are exactly the same for arrays and objects, as shown in the previous section.
    ...
    \begin{subfigure}[t]{0.495\linewidth}
        ...
        \resizebox{.9\linewidth}{!}{
            \begin{minipage}{\linewidth}
                \begin{alignat*}{5}
                    &\textbf{(1)}\quad&&\cdots\quad &&\quad && &&\text{; Code for $r_p \leftarrow \llbracket addr(x) \rrbracket$}\\
                    &\textbf{(2)}\quad&&\inst{exch}\quad && r_t\quad && r_p\quad &&\text{; extract size from object}\\
                    &\textbf{(3)}\quad&&\cdots\quad && \quad && \quad &&\text{; Code for $r_v \leftarrow \llbracket e \rrbracket$}\\
                    &\textbf{(4)}\quad&&\inst{xori}\quad && r_t\quad && r_v &&\text{; clear address of vtable in $r_t$}\\
                    &\textbf{(5)}\quad&&\inst{addi}\quad && r_p\quad && offset_{ref}\quad &&\text{; Index to ref count pos}\\
                    &\textbf{(6)}\quad&&\inst{exch}\quad && r_t\quad && r_p\quad &&\text{; Extract ref count}\\
                    &\textbf{(7)}\quad&&\inst{xori}\quad && r_t\quad && 1 &&\text{; Clear ref count}\\
                    &\textbf{(8)}\quad&&\inst{subi}\quad && r_p\quad && offset_{ref}\quad &&\text{; Inverse of \textbf{(5)}}\\
...
While a program written in \rooplpp might be syntactically valid and well-typed, it is not a guarantee that it executes successfully. A number of conditions exists, which cannot be determined at compile time, which results in erroneous compiled and executed code. \citeauthor{th:roopl} describes the following conditions:
