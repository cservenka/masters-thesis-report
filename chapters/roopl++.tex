...
\begin{subfigures}
    ...
    \begin{figure}[!ht]
        \begin{center}   
            ...
            \AXC{$r \notin\ \text{dom}(\mu)$}
            \DP \vskip 1em
            \AXC{$\mu' = \mu[r \mapsto v_1]$}
            \AXC{$\gamma[x \mapsto r] \vdash^{\Gamma}_{stmt}\ s : \mu' \rightleftharpoons \mu''$}
            \AXC{$\mu'' = \mu'[r \mapsto v_2]$}
            \AXC{$\mu''' = \mu'' \setminus \{ r \}$}
            \RL{\textsc{LocalBlock}}
            \QIC{$\gamma \vdash^{\Gamma}_{stmt}\ \textbf{local}\ c\ x = e_1 \quad s \quad \textbf{delocal}\ x = e_2\ :\ \mu \rightleftharpoons \mu'''$} 
            \DP
        \end{center}  
...
Local blocks are as previously mentioned, semantically similar to object blocks, where the memory location of variable $x$ must be unused in the store $\mu$. The updated store $\mu'$ contains location $r$ mapped to the evaluated value of $e_1$, $v_1$ and the reference $r$, which $x$ is bound to. The result store after body statement execution, $\mu''$ must have $r$ mapped to the expression value of $e_2$, $v_2$. Before the local block terminates, a third store update is executed, clearing the used memory locations, such that and $r$ are mapped to zero and become unused again.
